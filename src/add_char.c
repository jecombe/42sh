...
/*   Updated: 2018/06/23 11:53:22 by dewalter    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int		add_char_into_line(char key, t_shell *sh, t_editor *ed)
{
	...
	if ((!((ft_strlen(sh->line) + ed->prompt_size) % sz.ws_col) && ed->last_row == sz.ws_row))
	{
	...
	else if (!((ft_strlen(sh->line) + ed->prompt_size) % sz.ws_col))
	{
		ft_putchar(key);
		if (!(get_cursor_position(0) % sz.ws_col))
			tputs(tgetstr("do", NULL), 1, ft_putchar);
	...
	ft_strcat(tmp, sh->line + ed->cursor_str_pos - 1);
	ft_strdel(&sh->line);
	sh->line = ft_strdup(tmp);
	tputs(tgetstr("sc", NULL), 1, ft_putchar);
	ft_putstr(sh->line + ed->cursor_str_pos);
