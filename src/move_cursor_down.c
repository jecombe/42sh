...
/*   Updated: 2018/06/19 08:44:27 by dewalter    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void	move_cursor_down(t_shell *sh, t_editor *ed)
{
	...
	{
		if (get_cursor_position(1) + 1 == ed->last_row && (ft_strlen(sh->line
		+ ed->cursor_str_pos) - (sz.ws_col - get_cursor_position(0))
		< get_cursor_position(0)))
		{
			tputs(tgoto(tgetstr("ch", NULL), 0, (ft_strlen(sh->line +
			ed->cursor_str_pos) - (sz.ws_col - get_cursor_position(0))) - 1), 1,
			ft_putchar);
			ed->cursor_str_pos = ft_strlen(sh->line);
		}
		else
