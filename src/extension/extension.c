...
/*   Updated: 2018/08/29 05:52:21 by gmadec      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static int	init_parcour_tab(t_bquote **i, int *dquote)
{
...
static int	ft_parcour_tab(char ***cmd)
{
	...
	if (*cmd)
		while (*cmd && (*cmd)[++i->i])
		{
			...
			while ((*cmd)[i->i][i->j])
			{
				if ((ret = backslash_manager(&(*cmd), &i, dquote)))
					;
				else if ((ret = ft_manage_quote(cmd, &i, &dquote)))
					;
				else
